...
def dificuldade():
   dificuldade = fonte.render("A dificuldade aumentou", 1, (255,255,255))
   tela.blit(dificuldade, (meia_largura, meia_altura))
...
class Monster():
  ...
  def faster():
   ...
   if posmons[1] <= 0:
       subir = False
   if virar == True:
       posmons[0] -= velocidade
   else:
       posmons[0] += velocidade
   if subir == True:
       posmons[1] -= velocidade
   else:
       posmons[1] += velocidade
  def dead():
      global dead, monster
      ...
      dead = False
  def detect_collide():
   global char,monster,posmons,dead
...
while game_on == True:
   ...
   if level <= 10:
       ...
       Monster.faster()
   ##### a frase ta aparecendo toda hora, corrigir isso#####
   check_multiple10 = int(level/10)
   if check_multiple10 > 0 and isinstance(check_multiple10,int) == True:
       dificuldade()
